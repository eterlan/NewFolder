...
{
    [MenuItem("Tools/Cy/移除选中物体上所有丢失引用的脚本")]
    public static void FindAndRemoveMissingInSelected()
    {
        FindAndRemoveMissingScripts(Selection.gameObjects);
    }
    
    public static void FindAndRemoveMissingScripts(IEnumerable<GameObject> gos)
    {
        // EditorUtility.CollectDeepHierarchy does not include inactive c$$anonymous$$ldren
        var deeperSelection = gos.SelectMany(go => go.GetComponentsInChildren<Transform>(true)).Select(t => t.gameObject);
        ...
        foreach (var go in deeperSelection)
        {
            ...
            if (count > 0)
            {
                if (PrefabUtility.IsPartOfAnyPrefab(go))
                {
                    ...
                    if (count == 0)
                        continue;
                    // if not the missing scripts must be prefab overrides on t$$anonymous$$s instance
                }
