...
public class VFXToolWindow : EditorWindow
{
    ...
    private void OnEnable()
    {
        ...
        EditorJsonUtility.FromJsonOverwrite(EditorPrefs.GetString(name), this);
    }
    
    private void OnDisable()
    {
        EditorPrefs.SetString(name, EditorJsonUtility.ToJson(this));
    }
    ...
    private void Buttons()
    {
        ...
        var fixSelectVFedX       = GUILayout.Button("处理选中特效");
        ...
        if (fixSelectVFedX || fixSelectFolderVFX)
        {
            var gos    = (fixSelectVFedX ? GetAllSelectedVFX() : GetAllVFXInFolder()).ToList();
            var psList = gos.Where(go => go.GetComponentsInChildren<ParticleSystem>().Length != 0).ToList();
            
            DeleteSelectedVFXMissingScripts(gos);
            foreach (var particleGo in psList)
            {
                if (!particleGo.TryGetComponent(out ParticleSystem _)) 
                    AddEmptyParticleComponent(particleGo);
                ResetPosition(particleGo.transform);
        
                var childrenPs = particleGo.GetComponentsInChildren<ParticleSystem>(true);
        ...
        void AddEmptyParticleComponent(GameObject particleGo)
        {
            Debug.LogWarning($"此物体: {particleGo} 没有特效组件, 尝试自动设置");
    ...
    private void Header()
    {
        ...
        if (m_foldoutHeader)
        {
            ...
            EditorGUILayout.PrefixLabel("层级相关");
            EditorGUILayout.LabelField("sdad ");
            EditorGUILayout.Separator();
            EditorGUI.indentLevel--;
        }
    ...
    {
        EditorGUILayout.Separator();
        EditorGUILayout.LabelField("显示顺序");
            EditorGUI.indentLevel++;
